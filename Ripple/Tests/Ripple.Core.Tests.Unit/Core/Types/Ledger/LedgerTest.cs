namespace Ripple.Core.Tests.Unit.Core.Types.Ledger
{
    using NUnit.Framework;

    using Ripple.Core.Core.Binary;
    using Ripple.Core.Core.Serialized;

    [TestFixture]
    public class LedgerTest
    {
        /**
         * ➜  rippled git:(develop) ✗ ~/rocksdb/ldb --hex --db=/mnt/rippled/db/rocksdb get 0x498ef6300a8693c98be8345037843a95216fb254b672af679c8ef1c85d213972
         .... {some long hex string} prefixed with 0x...

         */
        [Test]
        public void TestParsing()
        {
            string hex;
            // This is the format as returned by ldb, with an `0x` prefix
            hex = "";
            hex = hex.Substring(2);

            var parser = new BinaryParser(hex);
            var reader = new BinaryReader(parser);

            Assert.AreEqual(4403788, reader.UInt32().LongValue());
            Assert.AreEqual(4403788, reader.UInt32().LongValue());
            Assert.AreEqual(3, parser.ReadOne());

            Ripple.Core.Core.Types.Ledger.Ledger ledger = Ripple.Core.Core.Types.Ledger.Ledger.FromParser(parser);
            byte[] o = ledger.Version.ToByteArray();

            Assert.AreEqual('M', ((char)o[0]));
            Assert.AreEqual('I', ((char)o[1]));
            Assert.AreEqual('N', ((char)o[2]));
            Assert.AreEqual('\0', ((char)o[3]));
        }
    }
}
